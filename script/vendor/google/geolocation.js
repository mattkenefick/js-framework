(function() {
    ...
    function g(a, c, f) {
        if (!window['google']) {
            console.warn("Google not loaded. Canceling.");
        ...
        var h = {
            ...
            flat: b,
            icon: {
                url       : "image/markers/location.svg",
        ...
        this.set("position_options", {enableHighAccuracy: b,maximumAge: 1E3});
        this.a.bindTo("map", this.b);
        a && this.c(a)
    }
    g.prototype = window['google'] ? new google.maps.MVCObject : {};
    g.prototype.set = function(a, c) {
        if (/^(?:position|accuracy)$/i.test(a))
    ...
    function l(a) {
        navigator.geolocation && (a.e = navigator.geolocation.watchPosition(function(c) {
            a: {
                var f = new google.maps.LatLng(c.coords.latitude, c.coords.longitude), h = a.b.getMap() == d;
                if (h) {
                    if (a.d() != d && c.coords.accuracy > a.d())
                        break a;
                    a.b.setMap(a.map);
                    ...
                    google.maps.event.trigger(a.map, "position", f);
                }
    ...
    var m = {map: b,position: b,radius: b};
